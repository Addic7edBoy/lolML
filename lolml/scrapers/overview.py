...
class ScrapeOverview(object):
    def __init__(self, year='2019', leagues=['LCS', 'LCK', 'LEC', 'LMS', 'WC', 'MSI'], saveHtml='../temp/html_temp/', saveData='../temp/' + year + '/'):
        self.year = year
    ...
    def get_stats(self, filename):
        ...
        game_time = soup.find(
            'div', {'class': ['col-6 text-center']}).find('h1').text
        t_name1 = soup.find('a', {'href': re.compile(
            r'/tournament/tournament-stats/')}).text
        match_date = soup.find(
            'div', {'class': 'col-12 col-sm-5 text-right'}).text
        match_date = re.split(' ', match_date)[0]
        print(match_date)
        all_stats = soup.find_all('div', {'class': ['row rowbreak pb-4']})
        teams_stats = all_stats[1].find_all(
            'table', {'class': ['playersInfosLine footable toggle-square-filled']})
        ...
        objective_blue = all_stats[0].find(
            'div', {'class': ['col-12 col-sm-6']})
        objective_red = objective_blue.parent.findNext(
            'div').findNext('div', {'class': 'col-12 col-sm-6'})
        ...
        blue_team_name = all_stats[0].find(
            'div', {'class': 'blue-line-header'}).text
        red_team_name = all_stats[0].find(
            'div', {'class': 'red-line-header'}).text
    ...
    def download_html(self):
        soup = self.get_html_soup(
            'https://gol.gg/tournament/list/region-ALL/league-1/')
        tournaments = soup.find('table', {'class': 'table_list'})

        # line_count = 1
        for tournament in tournaments.find_all('a'):
            if re.search(self.year, tournament) and not re.match(r'LPL', tournament):
                link = tournament.get('href')
                t_name = tournament.text
                path = self.saveHtml + str(t_name)
                    ...
                    if match.find('td', {'class': 'text_victory'}) or (match.find('td', {'class': 'text_defeat'})):
                        ...
                        if self.game_bo1(match) == 1:
                            ...
                            urllib.request.urlretrieve(
                                download_url, self.saveHtml + str(t_name) + '/' + str(match_name))
                            print(
                                'match № {0}, --> {1} downloading'.format(match_count, match_name))
                            match_count += 1
                        else:
                            start_url = re.findall(
                                r'\d+', match.find('td', {'class': 'text-left'}).find('a').get('href'))
                            for url_plus in range(self.game_bo1(match)):
                                download_url = 'http://gol.gg/game/stats/' + \
                                    str(int(start_url[0]) +
                                        url_plus) + '/page-game/'
    ...
    def main(self):
        results = []
        for directory in os.listdir(self.saveHtml):
            for filename in os.listdir(self.saveHtml + directory + '/'):
                results.extend(self.get_stats(
                    self.saveHtml + directory + '/' + filename))

        with open(self.saveData + self.year + '/games_data.json', 'w') as fp:
